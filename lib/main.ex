defmodule Main do
  ...
  def terminate_child(name) do
    ...
    case :ranch.stop_listener(ranch_listener_name) do ## This function does not return until the listener is completely stopped.
        :ok -> :ok
        {:error, :not_found} -> Logger.info("Could not find ranch listenser: #{inspect ranch_listener_name}")
    end
    case Supervisor.terminate_child(@name, connection_listener_name) do
        :ok -> :ok
        {:error, :not_found} -> Logger.info("Could not find child: #{inspect connection_listener_name}")
    end
  end
end
