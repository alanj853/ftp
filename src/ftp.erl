...
-export([cd/2, close/1, delete/2, formaterror/1,
	 ...
	 open/1, open/2,
	 pwd/1, quote/2,
	 recv/2, recv/3, recv_bin/2, recv_bin/3,
...
-spec recv_bin(Pid        :: pid(),
	       RemoteFile :: string(), Offset :: integer()) ->
    {'ok', Bin :: binary()} |
	{'error', Reason :: restriction_reason() | common_reason()}.
...
handle_call({_, {recv_bin, RemoteFile, Offset}}, From, #state{chunk = false} =
	    State) ->
    setup_data_connection(State#state{caller = {recv_bin, RemoteFile, Offset},
				      client = From});
